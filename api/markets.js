...
const STALE_MAX_AGE_MS = 1000 * 60 * 30;
...
function rememberSnapshot(markets, source, request) {
  lastGoodSnapshot = {
    markets,
    source,
    request,
...
function getStaleSnapshot() {
  ...
  const ageMs = Date.now() - new Date(lastGoodSnapshot.fetchedAt).getTime();
  if (!Number.isFinite(ageMs) || ageMs > STALE_MAX_AGE_MS) return null;
  return {
    ...lastGoodSnapshot,
    ageMs,
  };
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    rememberSnapshot(markets, source, request);
    ...
    if (stale) {
      res.status(200).end(
        JSON.stringify({
          ...
          meta: {
            source: `${stale.source} (cache de contingência)`,
            fetchedAt: stale.fetchedAt,
            count: stale.markets.length,
            request: stale.request,
            degraded: true,
            staleAgeMs: stale.ageMs,
            warning: 'Gamma API indisponível; exibindo último snapshot válido.',
            upstreamError: error.message || 'Falha ao consultar o Polymarket.',
          },
        })
      );
      return;
